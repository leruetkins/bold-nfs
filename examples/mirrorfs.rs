...
use nfsserve::rpcbind_listener::PortmapTcpListener;
use nfsserve::nfs_udp_listener::NfsUdpListener;

#[cfg(unix)]
use std::os::unix::ffi::OsStrExt;

// Cross-platform helpers to convert between &[u8] and OsString/OsStr
#[cfg(unix)]
...
#[cfg(not(unix))]
fn os_from_bytes(bytes: &[u8]) -> OsString {
    // On Windows, perform lossy UTF-8 conversion.
    // NFS filenames are opaque bytes; this preserves most common UTF-8 names.
...
impl FSMap {
    ...
    async fn find_child(&self, id: fileid3, filename: &[u8]) -> Result<fileid3, nfsstat3> {
        let mut name = self
            ...
            .name
            .clone();
        let filename_os = os_from_bytes(filename);
        name.push(
            self.intern
                .check_interned(&filename_os)
                .ok_or(nfsstat3::NFS3ERR_NOENT)?,
        );
...
impl MirrorFS {
    ...
    async fn create_fs_object(
        ...
        let objectname_osstr = os_from_bytes(objectname);
        ...
        match object {
            ...
            CreateFSObject::Symlink((_, target)) => {
                ...
                #[cfg(not(unix))]
                {
                    // Windows symlink support requires elevated privileges and has separate APIs
                    // for files vs dirs; not reliably supported here.
...
impl NFSFileSystem for MirrorFS {
    ...
    async fn lookup(&self, dirid: fileid3, filename: &filename3) -> Result<fileid3, nfsstat3> {
        ...
        let objectname_osstr = os_from_bytes(filename);
        path.push(&objectname_osstr);
        if !exists_no_traverse(&path) {
    ...
    async fn readdir(
        ...
        for i in children.range((range_start, Bound::Unbounded)) {
            ...
            ret.entries.push(DirEntry {
                fileid,
                name: os_to_bytes(name.as_os_str()).into(),
                attr: fileent.fsmeta,
            });
    ...
    async fn write(&self, id: fileid3, offset: u64, data: &[u8]) -> Result<fattr3, nfsstat3> {
        ...
        let meta = f.metadata().await.or(Err(nfsstat3::NFS3ERR_NOENT))?;
    ...
    async fn rename(
        ...
        from_path.push(os_from_bytes(from_filename));
        ...
        to_path.push(os_from_bytes(to_filename));
        ...
        let oldsym = fsmap
            .intern
            .intern(os_from_bytes(from_filename))
            .unwrap();
        let newsym = fsmap
            .intern
            .intern(os_from_bytes(to_filename))
            .unwrap();
    ...
    async fn readlink(&self, id: fileid3) -> Result<nfspath3, nfsstat3> {
        ...
        if path.is_symlink() {
            if let Ok(target) = path.read_link() {
                Ok(os_to_bytes(target.as_os_str()).into())
            } else {
...
async fn main() {
    ...
    // Запускаем основной NFS-листенер
    let fs = MirrorFS::new(path.clone());
    let listener = NFSTcpListener::bind(&format!("0.0.0.0:{HOSTPORT}"), fs)
        .await
        .unwrap();

    // Узнаём фактический порт NFS (если занято 2049, ОС может выдать другой)
    let nfs_port = listener.get_listen_port();

    // Пытаемся запустить rpcbind/portmapper на 0.0.0.0:111.
    // Он будет отвечать PMAPPROC_GETPORT и отдавать порт основного NFS (nfs_port).
    // Важно: на Windows/Unix порт 111 может требовать прав администратора/быть занят.
    // В этом случае сервер продолжит работу без rpcbind, и потребуется mount с явными port= и mountport=.
    match PortmapTcpListener::bind("0.0.0.0:111", nfs_port, MirrorFS::new(path.clone())).await {
        Ok(pmap_listener) => {
            // Включаем RPCBIND по UDP (часто клиенты ходят именно по UDP на 111)
            if let Err(e) = pmap_listener.spawn_udp("0.0.0.0:111").await {
                warn!("Unable to bind rpcbind UDP on 0.0.0.0:111, continuing with TCP only. Error: {:?}", e);
            }
            // И TCP-слушатель rpcbind
            tokio::spawn(async move {
                if let Err(e) = pmap_listener.handle_forever().await {
                    warn!("rpcbind listener error on :111: {:?}", e);
                }
            });
        }
        Err(e) => {
            warn!("Unable to bind rpcbind on 0.0.0.0:111, continuing without rpcbind. Error: {:?}", e);
        }
    }

    // Попытаться запустить NFS по UDP (для клиентов, которые по умолчанию используют UDP)
    match NfsUdpListener::bind(&format!("0.0.0.0:{HOSTPORT}"), MirrorFS::new(path)).await {
        Ok(udp_listener) => {
            tokio::spawn(async move {
                if let Err(e) = udp_listener.handle_forever().await {
                    warn!("nfs/udp listener error on :{}: {:?}", HOSTPORT, e);
                }
            });
        }
        Err(e) => {
            warn!("Unable to bind NFS/UDP on 0.0.0.0:{}; continuing with TCP only. Error: {:?}", HOSTPORT, e);
        }
    }

    // Основной цикл NFS (TCP)
