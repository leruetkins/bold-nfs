use crate::server::filemanager::nfs::*;
use std::fs::Metadata;
use std::path::Path;
...
/// Converts NFS time to FileTime
fn nfstime3_to_filetime(nfstime: nfstime3) -> filetime::FileTime {
    let duration = std::time::Duration::new(nfstime.seconds as u64, nfstime.nseconds);
    filetime::FileTime::from_system_time(std::time::UNIX_EPOCH + duration)
}
...
pub async fn path_setattr(path: &Path, setattr: &sattr3) -> Result<(), nfsstat3> {
    match setattr.atime {
        ...
        set_atime::SET_TO_CLIENT_TIME(time) => {
            let _ = filetime::set_file_atime(path, nfstime3_to_filetime(time));
        }
        _ => {}
    };
    match setattr.mtime {
        ...
        set_mtime::SET_TO_CLIENT_TIME(time) => {
            let _ = filetime::set_file_mtime(path, nfstime3_to_filetime(time));
        }
        _ => {}
    };
    if let set_mode3::mode(mode) = setattr.mode {
        debug!(" -- set permissions {:?} {:?}", path, mode);
        let _mode = mode_unmask(mode);
        #[cfg(unix)]
        {
