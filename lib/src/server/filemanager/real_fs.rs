...
use crate::server::filemanager::nfs::fileid3;
 use crate::server::filemanager::nfs::nfsstat3;
 use crate::server::filemanager::nfs::ftype3;
 use crate::server::filemanager::nfs::sattr3;
 use crate::server::filemanager::nfs::nfspath3;
 use crate::server::filemanager::nfs::filename3;
use crate::server::filemanager::nfs::fattr3;
use crate::server::filemanager::fs_util::*;
use crate::server::filemanager::vfs::{DirEntry, NFSFileSystem, ReadDirResult, VFSCapabilities};
...
impl RealFS {
    pub fn new(root: PathBuf) -> RealFS {
        RealFS {
...
impl NFSFileSystem for RealFS {
    ...
    async fn read(
        ...
        // Проверим, существует ли файл и можем ли мы его открыть
        let metadata = match tokio::fs::metadata(&path).await {
            Ok(metadata) => metadata,
            Err(_) => return Err(nfsstat3::NFS3ERR_NOENT),
        };
        ...
        let mut f = match File::open(&path).await {
            Ok(file) => file,
            Err(_) => return Err(nfsstat3::NFS3ERR_ACCES), // Нет доступа к файлу
        };
